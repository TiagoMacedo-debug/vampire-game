document.addEventListener('DOMContentLoaded', () => {
    // --- 1. DEFINIÇÕES GLOBAIS E CONSTANTES (REBALANCEADO) ---
    const clans = {
        'Brujah': {
            class: 'card-brujah', name: 'Brujah', cost: 0,
            description: 'Membros da ralé, rebeldes e tiranos com uma paixão ardente.',
            initialBlood: 10, initialHumanity: 6,
            disciplines: [
                { name: 'Potência', cost: 1, effect: 'potency', description: 'Ignora a perda de Sangue ao enfrentar um perigo.' },
                { name: 'Presença', cost: 1, effect: 'presence', targetType: 'danger', description: 'Intimida um inimigo adjacente, fazendo-o fugir.' },
                { name: 'Rapidez', cost: 2, effect: 'celerity', description: 'Seu próximo movimento é silencioso e anula até 2 de dano.' }
            ]
        },
        'Nosferatu': {
            class: 'card-nosferatu', name: 'Nosferatu', cost: 75, // Custo reduzido
            description: 'Espiões e monstros escondidos, mestres dos segredos.',
            initialBlood: 8, initialHumanity: 7,
            disciplines: [
                { name: 'Ofuscação', cost: 2, effect: 'obfuscate', description: 'Move-se para uma casa adjacente sem ativar seu efeito.' },
                { name: 'Animalismo', cost: 1, effect: 'animalism', targetType: 'any', description: 'Transforma uma carta adjacente em Ratos.' },
                { name: 'Potência', cost: 1, effect: 'potency', description: 'Ignora a perda de Sangue ao enfrentar um perigo.' }
            ]
        },
        'Toreador': {
            class: 'card-toreador', name: 'Toreador', cost: 75, // Custo reduzido
            description: 'Artesãos obcecados pela beleza em todas as suas formas.',
            initialBlood: 8, initialHumanity: 7,
            disciplines: [
                { name: 'Presença', cost: 1, effect: 'charm', description: 'Encanta um Mortal, ganhando +2 de Sangue extra.' },
                { name: 'Auspícios', cost: 1, effect: 'auspex', description: 'Revela o tipo de todas as cartas no tabuleiro.' },
                { name: 'Rapidez', cost: 2, effect: 'celerity', description: 'Seu próximo movimento é silencioso e anula até 2 de dano.' }
            ]
        }
    };
    const cardPool = [
        { weight: 20, data: { name: 'Rato', type: 'animal', blood: 1 } }, // Chance reduzida
        { weight: 15, data: { name: 'Beco', type: 'safe', humanity: 1 } },
        { weight: 25, data: { name: 'Policial', type: 'danger', blood: -2 } },
        { weight: 15, data: { name: 'Mortal', type: 'mortal', blood: 3, humanity: -2 } },
        { weight: 8,  data: { name: 'O_Abraço', type: 'fateful_choice' } }, // Nova carta "O Abraço"
        { weight: 8,  data: { name: 'Carniçal', type: 'ally_choice' } },
        { weight: 7,  data: { name: 'Nova_Inquisicao', type: 'inquisition_danger', blood: -3 } },
        { weight: 1,  data: { name: 'Refugio', type: 'haven', blood: 4, humanity: 1 } },
        { weight: 1,  data: { name: 'Bolsa_sangue', type: 'special', blood: 4 } }
    ];

    // --- 2. VARIÁVEIS DE ESTADO E REFERÊNCIAS AO DOM ---
    let player; // ... (resto das variáveis de estado)
    
    // --- 3. DECLARAÇÃO DE TODAS AS FUNÇÕES ---

    // A. Funções de Setup, UI e Gerenciamento de Estado
    function createNewPlayer(clanName) {
        const clan = clans[clanName];
        return {
            clan: clan, blood: clan.initialBlood, humanity: clan.initialHumanity, position: 4,
            isDiseased: false, // Novo status de Doença
            runStats: { elitesDefeated: 0, turns: 0, influenceGained: 0, alliesMaintained: 0, embraced: 0 }
        };
    }
    // ... (resto das funções de setup)

    // B. Lógica do Modal (ATUALIZADA)
    function showChoiceModal(cardType, oldPosition) {
        const title = document.querySelector('#choice-modal h3');
        const desc = document.querySelector('#choice-modal p');
        const btn1 = document.getElementById('choice-btn-1');
        const btn2 = document.getElementById('choice-btn-2');

        if (cardType === 'ally_choice') { // Carniçal
            title.textContent = "Um Carniçal Leal";
            desc.textContent = "Você pode sacrificá-lo para limpar a área, ou reforçar sua lealdade para o futuro.";
            btn1.textContent = "Sacrificar (-2 Humanidade)";
            btn2.textContent = "Alimentar (-1 Sangue, +1 Influência)";
            btn1.onclick = () => resolveChoice('sacrifice', oldPosition);
            btn2.onclick = () => resolveChoice('maintain', oldPosition);
        } else if (cardType === 'fateful_choice') { // O Abraço
            title.textContent = "A Oportunidade de um Abraço";
            desc.textContent = "Este mortal... ele tem potencial. Amaldiçoá-lo com a imortalidade trará grande poder, a um custo.";
            btn1.textContent = "Ignorar";
            btn2.textContent = "Abraçar (-3 Sangue, -1 Humanidade, +5 Influência)";
            btn1.onclick = () => resolveChoice('ignore', oldPosition);
            btn2.onclick = () => resolveChoice('embrace', oldPosition);
        }
    ...
    function resolveChoice(choice, oldPosition) {
        ...
        switch(choice) {
            case 'sacrifice':
                player.humanity = Math.max(0, player.humanity - 2);
                message = "Você o envia para uma morte certa. O caminho está limpo.";
                getAdjacentIndices(player.position).forEach(idx => {
                    if (gameBoard[idx] && (gameBoard[idx].type.includes('danger') || gameBoard[idx].type === 'obstacle')) {
                        gameBoard[idx] = null; // Remove a carta
                    }
                });
                break;
            case 'maintain':
    ...
    // C. Ciclo de Jogo Principal (ATUALIZADO)
    function movePlayer(targetIndex) {
        // ... (lógica inicial de checagem)
        const targetCard = { ...gameBoard[targetIndex] };

        // Lógica de cartas de escolha
        if (targetCard.type === 'ally_choice' || targetCard.type === 'fateful_choice') {
            player.position = targetIndex;
            gameBoard[player.position] = null;
            showChoiceModal(targetCard.type, oldPosition);
            return;
        }
        ...
        // Lógica da Doença
        if (targetCard.type === 'animal') {
            if (Math.random() < 0.25) { // 25% de chance
                player.isDiseased = true;
                message += " O sangue da criatura estava doente! ";
            }
        }
        if (targetCard.type === 'mortal' || targetCard.type === 'haven') {
            if (player.isDiseased) {
                player.isDiseased = false;
                message += " O sangue puro te cura da doença. ";
            }
        }
        // ... (resto da lógica de interação com a carta)
    }

    function endTurn({ message, oldPosition }) {
        // NOVO: Efeito da Doença no final do turno
        if (player.isDiseased) {
            player.blood--;
            logMessage(message.trim() + " (Você se sente doente...)");
        } else {
            logMessage(message.trim());
        }

        // ... (resto da lógica de endTurn)
    }

    function endGame(endMessage) {
        // ... (cálculo de influência)
        // NOVO: Adiciona bônus do Abraço e de Carniçais
        const alliesBonus = player.runStats.alliesMaintained * 1;
        if(alliesBonus > 0) { runInfluence += alliesBonus; summaryHTML += `<p>+${alliesBonus} por manter seus aliados.</p>`; }
        
        const embracedBonus = player.runStats.embraced * 5;
        if(embracedBonus > 0) { runInfluence += embracedBonus; summaryHTML += `<p>+${embracedBonus} por expandir sua linhagem.</p>`; }
        
        // ... (resto da lógica de endGame)
    }
    
    // --- CÓDIGO FINAL COMPLETO ---
    // (Cole o JS completo da resposta anterior e aplique as mudanças acima para garantir)
    let weightedCardDeck = [], gameBoard = new Array(9).fill(null), activeDiscipline = null, isPlayerTurn = true, totalInfluence = 0, unlockedClans = {};
    ...
    function populateClanStore() {
        const container = document.getElementById('clans-store-container');
        container.innerHTML = '';
        for (const clanName in clans) {
            const clan = clans[clanName];
            const card = document.createElement('div');
            card.className = 'clan-store-card';

            // Monta a lista de disciplinas
            let disciplinesHtml = '<ul class="discipline-list">';
            clan.disciplines.forEach(disc => {
                disciplinesHtml += `<li><strong>${disc.name} (${disc.cost}):</strong> ${disc.description}</li>`;
            });
            disciplinesHtml += '</ul>';

            let buyButtonHtml = '';
            if (unlockedClans[clanName]) {
                card.classList.add('unlocked');
                buyButtonHtml = `<span><strong>Desbloqueado</strong></span>`;
            } else {
                card.classList.add('locked');
                buyButtonHtml = `<button class="buy-btn" data-clan="${clanName}" ${totalInfluence < clan.cost ? 'disabled' : ''}>Comprar (${clan.cost} Influência)</button>`;
            }
            ...
            card.innerHTML = `
                ...
                <h4>Disciplinas:</h4>
                ${disciplinesHtml}
                <div class="buy-section">${buyButtonHtml}</div>
            `;
            container.appendChild(card);
        }
        document.querySelectorAll('.buy-btn').forEach(btn => btn.addEventListener('click', () => buyClan(btn.dataset.clan)));
    }
    ...
    function movePlayer(targetIndex) { if (!isPlayerTurn || !gameBoard[targetIndex]) return; isPlayerTurn = false; player.runStats.turns++; const oldPosition = player.position; const targetCard = { ...gameBoard[targetIndex] }; if (targetCard.type === 'ally_choice' || targetCard.type === 'fateful_choice') { player.position = targetIndex; gameBoard[player.position] = null; showChoiceModal(targetCard.type, oldPosition); return; } const isElite = targetCard.type.includes('inquisition'); if (isElite) player.runStats.elitesDefeated++; const playerCardElement = document.querySelector('.player'); const targetCardElement = Array.from(boardElement.children)[targetIndex]; if (!playerCardElement || !targetCardElement) { isPlayerTurn = true; return; } playerCardElement.style.transform = `translate(${targetCardElement.offsetLeft - playerCardElement.offsetLeft}px, ${targetCardElement.offsetTop - playerCardElement.offsetTop}px)`; targetCardElement.style.opacity = '0'; setTimeout(() => { let message = ''; let isSilentMove = false; if (activeDiscipline) { player.blood -= activeDiscipline.cost; switch (activeDiscipline.effect) { case 'potency': if (targetCard.type.includes('danger')) { targetCard.blood = 0; message = 'Com Potência, você supera o perigo. '; } break; case 'obfuscate': message = 'Você usa Ofuscação e passa despercebido.'; isSilentMove = true; break; case 'celerity': isSilentMove = true; if(targetCard.blood < 0) {targetCard.blood = Math.min(0, targetCard.blood + 2);} message = 'Seu ataque é veloz e silencioso. '; break; } activeDiscipline = null; } if (targetCard.blood) { player.blood = Math.min(15, player.blood + targetCard.blood); } if (targetCard.humanity) { player.humanity = Math.max(0, Math.min(10, player.humanity + targetCard.humanity)); } if (targetCard.type === 'animal' && Math.random() < 0.25) { player.isDiseased = true; message += " O sangue da criatura estava doente! "; } if ((targetCard.type === 'mortal' || targetCard.type === 'haven') && player.isDiseased) { player.isDiseased = false; message += " O sangue puro te cura da doença. "; } let influenceFromAction = 0; if (targetCard.type.includes('danger') || targetCard.type === 'mortal') { influenceFromAction = 1; } else if (targetCard.type === 'safe' || targetCard.type === 'animal') { influenceFromAction = 0.5; } player.runStats.influenceGained += influenceFromAction; player.position = targetIndex; gameBoard[player.position] = null; if (targetCard.type === 'obstacle') { gameBoard[oldPosition] = getRandomCard(); endTurn({message, oldPosition: -1}); return; } if (!isSilentMove) { const isLoudAction = targetCard.type.includes('danger') || targetCard.type === 'mortal'; if (isLoudAction && Math.random() < 0.33) { const validNeighbors = getAdjacentIndices(targetIndex).filter(i => gameBoard[i] && i !== oldPosition); if (validNeighbors.length > 0) { gameBoard[validNeighbors[0]] = { name: 'Policial', type: 'danger', blood: -2 }; message += 'A comoção atrai a polícia!'; } } } endTurn({ message, oldPosition }); }, 300); }
    ...
    function startGame(clanName) { player = createNewPlayer(clanName); gameBoard.fill(null); fillBoard(); isPlayerTurn = true; activeDiscipline = null; logMessage(`A noite começa para o neófito do clã ${clanName}.`); const quitBtn = document.getElementById('quit-run-btn'); if(quitBtn) { quitBtn.textContent = "Desistir da Noite"; quitBtn.onclick = () => { endGame("Você abandonou a caçada."); }; } updateUI(); saveGame(); showScreen('game'); }
    function newGame() { localStorage.removeItem('vtm_save_game'); populateClanSelection(); showScreen('clanSelect'); }
    ...
    function init() {
        ...
        // Listeners dos botões do Menu Principal
        ...
        document.getElementById('clans-btn').addEventListener('click', () => {
            populateClanStore();
            showScreen('store');
        });
